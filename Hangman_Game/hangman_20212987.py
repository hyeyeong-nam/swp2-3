...
class HangmanGame(QWidget):
    ...
    def guessClicked(self):
        ...
        if not(len(guessedChar) == 1):
            self.message.setText("Please input 1 alphabet")
            return

        # 이미 사용한 글자인지를 판단하고, 아닌 경우 메시지 출력, 리턴
        if guessedChar in self.guess.guessedChars:
            self.message.setText("Please input new alphabet")
            return
        ...
        self.hangmanWindow.setPlaceholderText(self.hangman.text[self.hangman.getRemainingLives()])
